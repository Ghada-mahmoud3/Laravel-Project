<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('jobs.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 flex items-center">
        ...
        <form method="GET" action="{{ route('jobs.search') }}" class="space-y-6 bg-white p-6 rounded-xl shadow-md mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <input
                    ...
                    value="{{ request()->application_deadline }}"
                    class="rounded-lg border border-gray-300 shadow-sm w-full focus:ring-indigo-500 focus:border-indigo-500 text-gray-700"
                >
            ...
            @forelse ($jobs as $job)
                <div class="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-lg transition">
                    <div class="flex items-center mb-4">
                        @if($job->logo_path)
                            <img src="{{ asset('storage/' . $job->logo_path) }}" alt="{{ $job->title }} Logo" class="w-12 h-12 object-contain rounded-lg border border-gray-200 mr-3">
                        @else
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                        @if($job->salary_min && $job->salary_max)
                            <span class="text-sm font-semibold text-green-600">${{ number_format($job->salary_min) }} - ${{ number_format($job->salary_max) }}</span>
                        @elseif($job->salary_min)
                            <span class="text-sm font-semibold text-green-600">From ${{ number_format($job->salary_min) }}</span>
                        @elseif($job->salary_max)
                            <span class="text-sm font-semibold text-green-600">Up to ${{ number_format($job->salary_max) }}</span>
                        @else
                            <span class="text-sm text-gray-500">Salary not specified</span>
                        @endif

                        <a href="{{ route('jobs.show', $job->id) }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">View Details →</a>
                    </div>
                </div>
            @empty
